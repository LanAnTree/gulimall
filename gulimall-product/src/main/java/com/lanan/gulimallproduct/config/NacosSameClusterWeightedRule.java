...
/**
 * @desc NacosSameClusterWeightedRule
 *      自定义负载均衡实现需要实现 ReactorServiceInstanceLoadBalancer 接口
 *      以及重写choose方法
 * @author lanan
 * @date 2022-07-06 11:05:49
 **/
@Slf4j
public class NacosSameClusterWeightedRule implements ReactorServiceInstanceLoadBalancer {
    ...
    public Mono<Response<ServiceInstance>> choose(Request request) {
        ...
        return supplier.get(request)
                .next().map((serviceInstances) -> this.processInstanceResponse(supplier, serviceInstances));
    }
    ...
    private Response<ServiceInstance> getInstanceResponse(List<ServiceInstance> instances) {
        ...
        List<ServiceInstance> sameClusterNameInstList  = instances.stream()
                .filter(i-> StringUtils.equals(i.getMetadata().get("nacos.cluster"),currentClusterName))
    ...
    private ServiceInstance getHostByRandomWeight(List<ServiceInstance> sameClusterNameInstList){
        ...
        Map<String,ServiceInstance> dataMap = new HashMap<>(2);
        // 此处将 ServiceInstance 转化为 Instance
        // 是为了接下来调用nacos中的权重算法，由于入参不同，所以需要转换
        // 此处建议打断电进行参数调试，以下是我目前为止所用到的参数，转化为map是为了最终方便获取取值到的服务对象
